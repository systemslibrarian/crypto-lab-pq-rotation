 ...
 *     nothing, and every verdict here — the four tamper outcomes, the rotation
 *     run, the migration and CRQC projections — renders only after its control
 *     is driven.
 *
 ...
 * plausible offender: it prints ML-DSA signatures, which are kilobytes of hex,
 * and several grids collapse to a bare `1fr` under a media query.
 */
 ...
 *
 * All four tamper outcomes are driven, not just the clean one: the whole claim
 * of the hybrid certificate is that altering the body OR either signature is
 * caught, and each renders a separately styled verdict a first-paint-only gate
 * never reaches.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  await scan(page, `${theme} / first paint`);

  // The three preset inventories.
  for (const demo of ['small', 'finance', 'gov']) {
    await page.locator(`[data-demo="${demo}"]`).click();
    await expect(page.locator('#inventory')).not.toBeEmpty();
    await scan(page, `${theme} / inventory: ${demo}`);
  }

  // Certificate views: classical, PQ and hybrid are different renderings.
  for (const view of ["classical", "pure_pq", "hybrid"]) {
    const v = page.locator(`[data-view="${view}"]`);
    if ((await v.count()) === 0) {
      throw new Error(`cert view "${view}" not found — the gate must fail rather than skip it`);
    }
    await v.click();
    await scan(page, `${theme} / cert view: ${view}`);
  }

  // Every tamper branch, then back to clean.
  for (const t of ['body', 'classical', 'pq', 'none']) {
    await page.locator(`[data-tamper="${t}"]`).click();
    await expect(page.locator('#certs')).not.toBeEmpty();
    await scan(page, `${theme} / tamper: ${t}`);
  }

  await page.locator('[data-action="run-verify"]').click();
  await expect(page.locator('#verify')).not.toBeEmpty();
  await scan(page, `${theme} / verification run`);

  await page.locator('[data-action="run-rotation"]').click();
  await expect(page.locator('#rotation')).not.toBeEmpty();
  await scan(page, `${theme} / rotation run`);

  // The two projection sliders, each redrawing the timeline.
  await page.locator('#migration-slider').fill('7');
  await expect(page.locator('#timeline')).not.toBeEmpty();
  await scan(page, `${theme} / migration projection`);

  await page.locator('#crqc-slider').fill('2035');
  await scan(page, `${theme} / CRQC projection`);
}
